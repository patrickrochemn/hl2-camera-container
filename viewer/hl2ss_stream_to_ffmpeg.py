...
HOLOLENS_HOST = "192.168.9.38"  # Update with your HoloLens IP
...
def start_hololens_stream():
    # Attempt to start HoloLens stream
    try:
...
def show_test_screen():
    # Run FFmpeg to generate a test screen
    ffmpeg_process = subprocess.Popen([
        'ffmpeg', '-re', '-f', 'lavfi', '-i', f'testsrc=size={WIDTH}x{HEIGHT}:rate={FRAMERATE}',
        '-c:v', 'libx264', '-preset', 'ultrafast', '-f', 'rtsp', 'rtsp://rtsp-server:8554/hololens'
    ])
    
    # Show the test screen fora set duration
    try:
